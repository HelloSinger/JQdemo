...
package com.jq.code.view.timessquare;
...
public class CalendarPickerView extends ListView {
  ...
  final List<MonthCellDescriptor> selectedMonthCals = new ArrayList<>();
  ...
  List<Date> haveDates = new ArrayList<>() ;
  ...
  public FluentInitializer init(Date minDate, Date maxDate, List<Date> dates) {
    haveDates = dates ;
    return init(minDate, maxDate, TimeZone.getDefault(), Locale.getDefault());
  }
  ...
  public ArrayList<Date> getHaveDataSelectedDates() {
    ArrayList<Date> selectedDates = new ArrayList<>();
    for (MonthCellDescriptor cal : selectedMonthCals) {
      if(cal.isHaveData()){
  ...
  public ArrayList<Date> getAllSelectedDates(){
    ArrayList<Date> selectedDates = new ArrayList<>();
    for (MonthCellDescriptor cal : selectedMonthCals) {
      selectedDates.add(cal.getDate());
    }
  ...
  private class CellClickedListener implements MonthView.Listener {
    ...
    public void handleClick(MonthCellDescriptor cell) {
      ...
      if (!isDateSelectable(clickedDate) || !cell.isSelectable()) {
        ...
        if (dateListener != null) {
            dateListener.onDateClick(clickedDate);
        }
      }
    }
  }
  ...
  private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
    ...
    for (MonthCellDescriptor selectedCell : selectedMonthCals) {
    ...
    if (date != null) {
      // Select a new cell.
      if (selectedMonthCals.size() == 0 || !selectedMonthCals.get(0).equals(cell)) {
        if(selectedMonthCals.size() == 1 && cell.getDate().before(selectedMonthCals.get(0).getDate())){
          selectedMonthCals.add(0,cell);
          selectedCals.add(0,newlySelectedCal);
        }else {
          selectedMonthCals.add(cell);
          selectedCals.add(newlySelectedCal);
        }
        cell.setSelected(true);
      }
      if (selectionMode == SelectionMode.RANGE && selectedMonthCals.size() > 1) {
        // Select all days in between start and end.
        Date start = selectedMonthCals.get(0).getDate();
        Date end = selectedMonthCals.get(1).getDate();
        selectedMonthCals.get(0).setRangeState(RangeState.FIRST);
        selectedMonthCals.get(1).setRangeState(RangeState.LAST);
          ...
          for (List<MonthCellDescriptor> week : month) {
            for (MonthCellDescriptor singleCell : week) {
              if (singleCell.getDate().after(start)
                ...
                selectedMonthCals.add(singleCell);
              }
            }
          }
        }
  ...
  private Date applyMultiSelect(Date date, Calendar selectedCal) {
    for (MonthCellDescriptor selectedCell : selectedMonthCals) {
      if (selectedCell.getDate().equals(date)) {
        // De-select the currently-selected cell.
        selectedCell.setSelected(false);
        selectedMonthCals.remove(selectedCell);
        date = null;
        break;
      }
    }
  ...
  List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
    ...
    while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
      ...
      for (int c = 0; c < 7; c++) {
        ...
        boolean isSelectable = isCurrentMonth && isTodayBefer(cal) && isDateSelectable(date);
        ...
        weekCells.add(
            new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday,
                isHighlighted, value, rangeState,haveDates.contains(date)));
        cal.add(DATE, 1);
      }
    }
    return cells;
  ...
  public  boolean isTodayBefer(Calendar cal){
       Date curCal  = cal.getTime() ;
       return curCal.before(today.getTime()) ;
  }
