...
public class CropImageView extends ImageView {
    ...
    protected Rect mTmpDrawableFloat = new Rect();// 浮层的Rect
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mTmpDrawableFloat.left = mDrawableFloat.left;
                mTmpDrawableFloat.top = mDrawableFloat.top;
                mTmpDrawableFloat.right = mDrawableFloat.right;
                mTmpDrawableFloat.left = mDrawableFloat.left;
                mX_1 = event.getX();
                mY_1 = event.getY();
    ...
    private void checkBoundsEDGE() {
        ...
        if (mDrawableFloat.left < mDrawableDst.left) {
            mDrawableFloat.left = mDrawableDst.left;
            isChange = true;
        }

        if (mDrawableFloat.top < mDrawableDst.top) {
            mDrawableFloat.top = mDrawableDst.top;
            isChange = true;
        }

        if (mDrawableFloat.right > mDrawableDst.right) {
            mDrawableFloat.right = mDrawableDst.right > getWidth() ? getWidth() : mDrawableDst.right;
            isChange = true;
        }

        if (mDrawableFloat.bottom > mDrawableDst.bottom) {
            mDrawableFloat.bottom = mDrawableDst.bottom > getHeight() ? getHeight() : mDrawableDst.bottom;
            isChange = true;
        }

        if (mTmpDrawableFloat.left < mDrawableFloat.left) {
            if (isMinWidth()) {
                mDrawableFloat.left = mDrawableFloat.right - mFloatDrawable.getBorderWidth();
                isChange = true;
            }
        }

        if (mTmpDrawableFloat.top < mDrawableFloat.top) {
            if (isMinHeight()) {
                mDrawableFloat.top = mDrawableFloat.bottom - mFloatDrawable.getBorderHeight();
                isChange = true;
            }
        }
        if (mTmpDrawableFloat.right > mDrawableFloat.right) {
            if (isMinWidth()) {
                mDrawableFloat.right = mDrawableFloat.left + mFloatDrawable.getBorderWidth();
                isChange = true;
            }
        }

        if (mTmpDrawableFloat.bottom < mDrawableFloat.bottom) {
            if (isMinHeight()) {
                mDrawableFloat.bottom = mDrawableFloat.top + mFloatDrawable.getBorderHeight();
                isChange = true;
            }
        }
    ...
    private boolean isMinHeight() {
        return mDrawableFloat.height() < mFloatDrawable.getBorderHeight();
    }

    private boolean isMinWidth() {
        return mDrawableFloat.width() < mFloatDrawable.getBorderWidth();
    }
    ...
    protected void checkBounds() {
        ...
        boolean isChange = false;
        if (mDrawableFloat.left < mDrawableDst.left) {
            newLeft = mDrawableDst.left < 0 ? 0 : mDrawableDst.left;
            isChange = true;
        }

        if (mDrawableFloat.top < mDrawableDst.top) {
            newTop = mDrawableDst.top < 0 ? 0 : mDrawableDst.top;
            isChange = true;
        }

        if (mDrawableFloat.right > mDrawableDst.right) {
            newLeft = (mDrawableDst.right > getWidth() ? getWidth() : mDrawableDst.right) - mDrawableFloat.width();
            isChange = true;
        }

        if (mDrawableFloat.bottom > mDrawableDst.bottom) {
            newTop = (mDrawableDst.bottom > getHeight() ? getHeight() : mDrawableDst.bottom) - mDrawableFloat.height();
            isChange = true;
        }
