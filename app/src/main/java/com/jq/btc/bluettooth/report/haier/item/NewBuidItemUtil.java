...
/**
 * Create by AYD on 2018/12/24
 */
public class NewBuidItemUtil {
    ...
    public NewBuidItemUtil(Context context, RoleInfo roleInfo, CsAlgoBuilder csAlgoBuilder) {
...
//        this.mWeightEntity = entity;
     ...
     * 内脏脂肪
     */
    public IndexDataItem buildVisceraItem(float viscera) {
//        float viscera = mWeightEntity.getViscera();
     ...
     * 水分
     */
    public IndexDataItem buildWaterItem(float water) {
//        float water = mWeightEntity.getWater();
        ...
        float[] waterStandard = WeighDataParser.getWaterStandardRange(roleInfo.getSex(),
                TimeUtil.getAgeThroughBirthday(roleInfo.getBirthday()));
     ...
     * 水分重量
     */
    public IndexDataItem buildWaterWeightItem(float water, float Weight) {
//        float water = mWeightEntity.getWater();
        // 水含量
        float waterWeight = Weight * water / 100.f;
        ...
        String[] topStrWater = new String[levelNumsWater.length];
        for (int i = 0; i < levelNumsWater.length; i++) {
            topStrWater[i] = StandardUtil.getWeightExchangeValue(context,
                    Weight * levelNumsWater[i] / 100, "", (byte) 1);
        }
...
//    }
     ...
     * 体脂率
     */
    public IndexDataItem buildAxungeItem(float axunge) {
//        float axunge = mWeightEntity.getAxunge();
        ...
        float[] axungeStandard = WeighDataParser.getAxungeStandardRange(roleInfo.getSex(),
                TimeUtil.getAgeThroughBirthday(roleInfo.getBirthday()));
     ...
     * 脂肪重量
     */
    public IndexDataItem buildFatWeightItem(float axunge, float weight) {
//        float axunge = mWeightEntity.getAxunge();
        // 脂肪重量
        float axungeWeight = weight * axunge / 100.f;
        ...
        String[] topStr = new String[levelNums.length];
        for (int i = 0; i < levelNums.length; i++) {
            topStr[i] = StandardUtil.getWeightExchangeValue(context, weight * levelNums[i] / 100, "", (byte) 1);
        }
...
//    }
     ...
     * 骨量
     */
    public IndexDataItem buildBoneItem(float bone) {
//        float bone = mWeightEntity.getBone();
        ...
        float[] boneStandard = WeighDataParser.getBoneStandardRange(roleInfo.getSex(),
                TimeUtil.getAgeThroughBirthday(roleInfo.getBirthday()));
     ...
     * 肌肉率
     */
    public IndexDataItem buildMuscleItem(float muscles) {
        float muscle = muscles / 2;
        ...
        float[] muscleStandard = WeighDataParser.getMuscleStandardRange(roleInfo.getSex());
     ...
     * 肌肉重量
     */
    public IndexDataItem buildMuscleWeightItem(float muscle, float weigth) {
//        float muscle = mWeightEntity.getMuscle();

        // 肌肉重量
        float muscleWeight = weigth * muscle / 200.f;
        ...
        String[] topStr = new String[levelNums.length];
        for (int i = 0; i < levelNums.length; i++) {
            topStr[i] = StandardUtil.getWeightExchangeValue(context, weigth * levelNums[i] / 100, "", (byte) 1);
        }
     ...
     * 基础代谢
     */
    public IndexDataItem buildMetabolismItem(float metabolism) {
//        float metabolism = mWeightEntity.getMetabolism();
        ...
        float[] metabolismStandard = WeighDataParser.getMetabolismStandardRange(roleInfo.getSex(),
                TimeUtil.getAgeThroughBirthday(roleInfo.getBirthday()));
     ...
     * 肥胖度
     */
    public IndexDataItem buildCorpulentItem(float weight) {
        float corpulent;
//        if (mWeightEntity.getR1() > 0) {
//            corpulent = csAlgoBuilder.getOD();
//        } else {
        corpulent = CsAlgoBuilder.calOD(csAlgoBuilder.getH(), weight, csAlgoBuilder.getSex(),
...
//        }
     ...
     * BMI
     */
    public IndexDataItem buildBMIItem(float bmi) {
